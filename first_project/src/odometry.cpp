...
public:
    scout_odometry(double x, double y, double theta) {
        ...
        odometry_pub = pub_Node.advertise<first_project::CustomOdometry>("/my_odom", 50);
        test_pub = pub_Node.advertise<nav_msgs::Odometry>("/test_odom", 50);
    ...
    void publish_odometry() {
        first_project::CustomOdometry custom_odom;
        ros::Time current_time = ros::Time::now();
        geometry_msgs::TransformStamped odom_trans;
        geometry_msgs::Quaternion odom_quaternion = tf::createQuaternionMsgFromYaw(currentPose.theta);

        custom_odom.method = method;
        custom_odom.odom.pose.pose.position.x = currentPose.x;
        custom_odom.odom.pose.pose.position.y = currentPose.y;
        custom_odom.odom.pose.pose.position.z = 0.0;
        custom_odom.odom.pose.pose.orientation = odom_quaternion;

        custom_odom.odom.header.frame_id = "odom";
        custom_odom.odom.child_frame_id = "base_link";
        custom_odom.odom.twist.twist.linear.x = vel.speed_x;
        custom_odom.odom.twist.twist.linear.y = vel.speed_y;
        custom_odom.odom.twist.twist.angular.z = vel.omega;
        ...
        odometry_pub.publish(custom_odom);

        //Test odometry to make comparison using rviz
        test_pub.publish(custom_odom.odom);
    }
